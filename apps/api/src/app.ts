...
function isTrustedOrigin(origin: string, env: AppEnv) {
  ...
  try {
    ...
    const webHost = new URL(env.webOrigin).hostname;
    // Loopback aliases are interchangeable only when the product origin is also loopback.
    return isLoopbackHost(webHost) && isLoopbackHost(host);
  } catch {
    return false;
  }
}
